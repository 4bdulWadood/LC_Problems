/*
    ...
    Time Complexity : This is a balanced and perfect binary tree so the number of nodes will be 2^N, where N is the height of the tree and the length of the input array.
                        the time complexity to visit 2^N nodes will be O(2^N) , where N is the length of the input. Addtionally the data in the subset is being combined with 
                        set[depth] element for each recursive iteration. So in the worst case the time complexity is O(2^N*N)

    Space Complexity : Since we are performing recursion the memory stack will hold data from previous recursive calls.
                       In the worst case the memory stack will hold data from N values, where N is the length of the input. Each recursive iteration holds N values at worst case. The
